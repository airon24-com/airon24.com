	...
	var RbSlider = function( slider, $slider, userSettings, sliderUID ) {
			...
			slide: {
				...
				normal: {
					...
					setTransition: function( type, transitionProperties ){
						...
						// REMOVED in 6.0.2
						//rb.slider.$innerWrapper.css( 'background-color', 'transparent' );
			...
			layers: {
				...
				timeline: {
					...
					prepare: function(){
						...
						if( rb.slides.next.data.overflow && rb.slides.next.data.overflow !== 'hidden' ){
							rb.slider.$layersWrapper.addClass( 'rb-visible' );
							rb.slider.$slideBGWrapper.addClass( 'rb-visible' );
						}else{
							rb.slider.$layersWrapper.removeClass( 'rb-visible' );
							rb.slider.$slideBGWrapper.removeClass( 'rb-visible' );
						}
				...
				hover: {
					...
					createTimeline: function( $layer, layerData ){
						...
						// 		layerData.filter.transitions.hover = rb.transitions.layers.filters.createTransition(
						// 			layerData,
						// 			'hover',
						...
						// 	layerData.hoverTo.onUpdate = rb.transitions.layers.filters.animate;
		...
		rb.plugins = {
			...
			init: function( curPluginNameSpace, curPlugin, loadPluginStyle ){
				...
				if( window._RbSlider.checkVersions( rb.initializedPlugins[curPluginNameSpace].pluginData.requiredRBVersion, rb.plugin.version ) ){
					...
					console.error( rb.defaults.slider.errorText, curPluginNameSpace, 'plugin has not been loaded! Required RbThemeSlider version:', rb.initializedPlugins[curPluginNameSpace].pluginData.requiredRBVersion, '(you have:', rb.plugin.version + ')'  );
				}
				rb.o.plugins.splice( 0, 1 );
			},
			...
			set: {
				...
				global: function(){
					...
					/*
						...
						rb.o						: Working slider settings (user settings merged with defaults)
					*/
				...
				styles: function(){
					...
					//rb.slider.$hiddenWrapper = $( '<div class="rb-hidden"></div>' ).appendTo( $slider );
