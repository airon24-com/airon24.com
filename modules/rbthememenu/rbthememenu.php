...
class Rbthememenu extends Module
{
    ...
    public function getBlockProducts($products)
    {
        ...
        if (!is_array($products)) {
            ...
            foreach ($IDs as $ID) {
                if ($ID &&($tmpIDs = explode('-', $ID))) {
                    $obj_p = new Product($tmpIDs[0], true, $this->context->language->id, $this->context->shop->id);

                    if (isset($obj_p->id_product)) {
                        $products[] = array(
                            'id_product' => $tmpIDs[0],
                            'id_product_attribute' => !empty($tmpIDs[1])? $tmpIDs[1] : 0,
                        );
        ...
        if (!empty($products)) {
            ...
            foreach ($products as &$product) {
                ...
                $product['minimal_quantity'] = $p->minimal_quantity;
                $product['unit_price_ratio'] = $p->unit_price_ratio;
                $product['available_date'] = $p->available_date;
                $product['online_only'] = $p->online_only;
