...
ElementModel = Backbone.Model.extend( {
    ...
    renderRemoteServer: function()
    {
        ...
        if (data.widgetType == 'prestashop-widget-Category' ||
        	data.widgetType == 'prestashop-widget-ProductsListTabs' ||
            data.widgetType == 'prestashop-widget-ProductsList' ||
            data.widgetType == 'prestashop-widget-Category-Sub' ||
            data.widgetType == 'countdown' ||
            data.widgetType == 'testimonial' ||
            data.widgetType == 'prestashop-widget-CustomTpl' ||
            data.widgetType == 'prestashop-widget-modules' ||
            data.widgetType == 'prestashop-widget-Blog' ||
            data.widgetType == 'prestashop-widget-Menu' ||
            data.widgetType == 'prestashop-widget-Rbthemeslider' ||
            data.widgetType == 'prestashop-widget-Brands' ||
            data.widgetType == 'image-carousel' ||
            data.widgetType == 'instagram' ||
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    if (widgetType == 'countdown') {
                        let html_base = data.data;
                        this_fc.createCountDown(html_base, settings, this_fc);
                    } else {
                        this_fc.onRemoteGetHtml(data.data);
                    }
                }
            });
        }
    ...
    createCountDown: function(html_base, settings, this_fc)
    {
        let clock = setInterval(function() {
            let html = html_base; 
            let time = "2050/05/29 00:00:00" ["2050", "05", "29", "00", "00", "00"];
            ...
            html += '<div class="rb-widget-container" style="text-align:center;">';
            html += '<div class="rb-countdown row">';

            if (settings.title && settings.title != '') {
                html += '<div class="rb-coundown-title">' +settings.title + '</div>';
            }

            html += '<ul class="rb-clock">';
            html += '<li ><span class="rb-days">'+days+'</span><span> Days</span></li>';
            html += '<li ><span class="rb-hours">'+hours+'</span><span> Hours</span></li>';
            html += '<li ><span class="rb-minutes">'+minutes+'</span><span> Minutes</span></li>';
            html += '<li ><span class="rb-seconds">'+seconds+'</span><span> Seconds</span></li>';
            html += '</ul>';

            if (settings.description && settings.title != '') {
                html += '<div class="rb-coundown-description">' + settings.description + '</div>';
           	}
            ...
            if (distance < 0) {
                clearInterval(clock);
                let expired = '<div class="rb-widget-container" style="text-align:center;">';
                expired += '<div class="rb-countdown row">';
                expired += '<h1>EXPIRED TIME<h1>';
                expired += '</div>';
                expired += '</div>';
                this_fc.onRemoteGetHtml(expired);
            }

            this_fc.onRemoteGetHtml(html);
        }, 1000);

        setTimeout(function() {
            for (var i = 1; i < clock; i++) {
        ...
        self.$el.imagesLoaded().always( function() {
            setTimeout( function() {
                if (self.model.getIcon() == 'instagram') {
                    let $instagramWrapper = self.$el.find('.rb-instagram');
                    let $carousel = self.$el.find('.rb-instagram-carousel');

                    if (!$instagramWrapper.length ) {
                        return;
                    }

                    let options = $instagramWrapper.data('options');
                    ...
                    $instagramWrapper.instagramLite({
                        ...
                        success: function() {
                            ...
                            let savedOptions = $carousel.data('slider_options');
