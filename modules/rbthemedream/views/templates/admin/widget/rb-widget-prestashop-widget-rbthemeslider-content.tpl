	...
	{foreach from=$sliders item="slider"}
		<div class="rb-slider-item" style="position:relative;">
			...
			{if !empty($slider.layers)}
				{foreach from=$slider.layers item=layer}
					{if isset($layer.text)}
						<div style="{$layer.rb_styles nofilter}">{$layer.text}</div>
					{/if}

					{if isset($layer.button)}
						<button class="btn" style="{$layer.rb_styles nofilter}">
							{$layer.button nofilter}
						</button>
					{/if}
				{/foreach}
			{/if}
