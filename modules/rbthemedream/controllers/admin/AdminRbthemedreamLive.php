...
class AdminRbthemedreamLiveController extends ModuleAdminController
{
    ...
    public function setMedia($isNewTheme = false)
    {
        ...
        $this->addCSS(array(
            ...
            _MODULE_DIR_.'rbthemedream/views/css/lib/rb-img-360.css',
            _MODULE_DIR_.'rbthemedream/views/css/lib/rb-prism.css',
        ));
        ...
        $this->addJS(array(
            ...
            _MODULE_DIR_.'rbthemedream/views/js/plugin/rb-instagram.js',
            ...
            _MODULE_DIR_.'rbthemedream/views/js/plugin/rb-img-360.js',
            _MODULE_DIR_.'rbthemedream/views/js/plugin/rb-prism.js',
    ...
    public function display()
    {
        ...
        if (Configuration::get('PS_REWRITING_SETTINGS') != 1) {
            $view_link = $this->context->shop->getBaseURL(true, true) . 'index.php?fc=module&module=rbthemedream&controller=view&page='.$type.'&'.$id.'='. Tools::getValue($id);
        }
        ...
        Media::addJsDef(array(
            ...
            'rbConfig' => array(
                ...
                'widgets' => array(
                    ...
                    'vertical-tabs' => (new RbVerticalTabs())->getDataVerticalTab(),
                    ...
                    'rb_img_360' => $rb_img_360->getDataImage360(),
                    'anchor' => (new RbAnchor())->getDataAnchor(),
                    'download' => (new RbDownLoad())->getDataDownLoad(),
                    'user-rating' => (new RbUserRating())->getDataUserRating(),
                    ...
                    'prestashop-widget-Category-Sub' => ((new RbPrestashopWidgetCategorySub()))->getDataCategorySub(),
                ),
                'schemes' => array(
                    'items' => array(
                        'color' => array(
    ...
    public function ajaxProcessRenderWidget()
    {
        ...
        if (!empty($data['settings'])) {
            ...
            $html = $this->module->fetch('module:rbthemedream/views/templates/admin/widget/rb-base.tpl');

            switch ($data['widgetType']) {
                case 'prestashop-widget-Category':
                    $obj_cate = new RbCategory();
                    $html .= $obj_cate->rbRender($settings);

                    break;
                case 'video':
                    $obj_video = new RbVideo();
                    $html .= $obj_video->rbRender($settings);

                    break;
                case 'testimonial':
                    $obj_testimonial = new RbTestimonial();
                    $html .= $obj_testimonial->rbRender($settings);

                    break;
                case 'prestashop-widget-CustomTpl':
                    $obj_tpl = new RbTpl();
                    $html .= $obj_tpl->rbRender($settings);

                    break;
                case 'prestashop-widget-Blog':
                    $obj_blog = new RbBlog();
                    $html .= $obj_blog->rbRender($settings);

                    break;
                case 'prestashop-widget-Menu':
                    $obj_menu = new RbMenu();
                    $html .= $obj_menu->rbRender($settings);

                    break;
                case 'prestashop-widget-ProductsListTabs':
                    Context::getContext()->customer = new Customer();
                    $obj_ptab = new RbProductTab();
                    $html .= $obj_ptab->rbRender($settings);

                    break;
                case 'prestashop-widget-ProductsList':
                    Context::getContext()->customer = new Customer();
                    $obj_p = new RbProduct();
                    $html .= $obj_p->rbRender($settings);

                    break;
                case 'prestashop-widget-Rbthemeslider':
                    $obj_slider = new RbSliderDD();
                    $html .= $obj_slider->renderAdmin($settings);

                    break;
                case 'prestashop-widget-Brands':
                    $obj_brand = new RbBrands();
                    $html .= $obj_brand->rbRender($settings);

                    break;
                case 'image-carousel':
                    $obj_img_c = new RbImageCarousel();
                    $html .= $obj_img_c->rbRender($settings);

                    break;
                case 'instagram':
                    $obj_insta = new RbInstagram();
                    $html .= $obj_insta->rbRender($settings);

                    break;
                case 'prestashop-widget-modules':
                    if ($settings['module'] == 'ps_categorytree' ||
                        $settings['module'] == 'ps_featuredproducts' ||
                        $settings['module'] == 'ps_sharebuttons'
                    ) {
                        $html .= '<div class="rb-widget-container"><h1 class="alert alert-danger">'.$this->module->l('Can not display this module. Please save and see on homepage').'</h1><div>';
                    } else {
                        $this->context->smarty->assign(array(
                            'urls' => array(
                                'pages' => array(
                        ...
                        $obj_module = new RbModule();
                        $html .= $obj_module->rbRender($settings);
                    }

                    break;
                case 'prestashop-widget-Category-Sub':
                    $obj_insta = new RbPrestashopWidgetCategorySub();
                    $html .= $obj_insta->rbRender($settings);
