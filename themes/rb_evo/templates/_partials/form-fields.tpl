...
{else}
  ...
  <div class="form-group row align-items-center {if !empty($field.errors)}has-error{/if}">
    <label class="col-md-2 col-form-label{if $field.required && $field.type !== 'checkbox'} required{/if}" for="field-{$field.name}">
      ...
      {elseif $field.type === 'checkbox'}
        ...
        {block name='form_field_item_checkbox'}
          <span class="custom-checkbox">
            ...
            <label class="{if $field.required} required{/if}" for="ff_{$field.name}">{$field.label nofilter}</label >
          </span>
        {/block}
