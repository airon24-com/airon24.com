...
* @author PrestaShop SA and Contributors <contact@prestashop.com>
  ...
  *}
  ...
  <article class="js-address-item address-item{if $address.id == $selected} selected{/if}" id="{$name|classname}-address-{$address.id}">
    <header class="h4">
    ...
    <footer class="address-footer">
      ...
      <a class="edit-address text-muted" data-link-action="edit-address"
        ...
        <i class="fa fa-pencil-square-o edit"></i>{l s='Edit' d='Shop.Theme.Actions'}
      </a>
      <a class="delete-address text-muted" data-link-action="delete-address"
        href="{url entity='order' params=['id_address' => $address.id, 'deleteAddress' => true, 'token' => $token]}">
        <i class="fa fa-trash-o delete"></i>{l s='Delete' d='Shop.Theme.Actions'}
      </a>
      {/if}
    </footer>
  </article>
  ...
  {if $interactive}
  <p>
    <button class="ps-hidden-by-js form-control-submit center-block" type="submit">{l s='Save'
